...
static int open_pipe(gesprog_pipe_t *pipe, const gesprog_server_config_t *config)
{
...
static int read_char(gesprog_pipe_t *pipe, char *ch)
{
    DWORD count = 0;
    return ReadFile(pipe->input, ch, 1, &count, NULL) && count == 1;
}

static int write_all(gesprog_pipe_t *pipe, const char *text, size_t len)
{
    DWORD count = 0;
    return WriteFile(pipe->output, text, (DWORD)len, &count, NULL) && count == len;
}

static void flush_pipe(gesprog_pipe_t *pipe)
{
    FlushFileBuffers(pipe->output);
}

static void close_pipe(gesprog_pipe_t *pipe)
{
    flush_pipe(pipe);
...
static int read_message(gesprog_pipe_t *pipe, char *buffer, size_t size)
{
    size_t used = 0;
    int too_long = 0;
...
static int write_message(gesprog_pipe_t *pipe, const char *message)
{
...
int gesprog_server_run(const gesprog_server_config_t *config)
{
    ...
    while (gesprog_service_state(&service) != GESPROG_SERVICE_TERMINADO) {
        ...
        } else if (!gesprog_service_handle_json(&service, config->aralmac, request,
