...
static int open_pipe(ejecutor_pipe_t *pipe, const ejecutor_server_config_t *config)
{
...
static int read_char(ejecutor_pipe_t *pipe, char *ch)
{
    DWORD count = 0;
    return ReadFile(pipe->input, ch, 1, &count, NULL) && count == 1;
}

static int write_all(ejecutor_pipe_t *pipe, const char *text, size_t len)
{
    DWORD count = 0;
    return WriteFile(pipe->output, text, (DWORD)len, &count, NULL) && count == len;
}

static void flush_pipe(ejecutor_pipe_t *pipe)
{
    FlushFileBuffers(pipe->output);
}

static void close_pipe(ejecutor_pipe_t *pipe)
{
    flush_pipe(pipe);
...
static int read_message(ejecutor_pipe_t *pipe, char *buffer, size_t size)
{
    size_t used = 0;
    int too_long = 0;
...
static int write_message(ejecutor_pipe_t *pipe, const char *message)
{
...
int ejecutor_server_run(const ejecutor_server_config_t *config)
{
    ...
    while (ejecutor_service_state(&service) != EJECUTOR_SERVICE_TERMINADO) {
        ...
        } else if (!ejecutor_service_handle_json(&service, config->aralmac, request,
